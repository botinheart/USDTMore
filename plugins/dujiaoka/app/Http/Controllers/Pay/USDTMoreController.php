...
class USDTMoreController extends PayController
{
    public function gateway(string $payway, string $orderSN)
    {
        try {
            ...
            //构造要请求的参数数组，无需改动
            $parameter = [
                "code" => $this->payGateway->pay_check,
                "amount" => (float)$this->order->actual_price,//原价
                "order_id" => $this->order->order_sn, //可以是用户ID,站内商户订单号,用户名
                'redirect_url' => route('usdtmore-return', ['order_id' => $this->order->order_sn]),
                'notify_url' => url($this->payGateway->pay_handleroute . '/notify_url'),
            ];
            $parameter['signature'] = $this->usdtmoreSign($parameter, $this->payGateway->merchant_id);
            $client = new Client([
    ...
    private function usdtmoreSign(array $parameter, string $signKey)
    {
        ksort($parameter);
    ...
    public function notifyUrl(Request $request)
    {
        ...
        $signature = $this->usdtmoreSign($data, $payGateway->merchant_id);
